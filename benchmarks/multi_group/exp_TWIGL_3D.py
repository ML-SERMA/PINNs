...
parser.add_argument('-t','--test', type=str, help="name of test case",default='EP_Mixed_FCN_2G_TWIGL_3D_SLR2e-4_09_Nit2000_HBC')
...
params_domain = {'xmin':[0, 0, 0], 'xmax':[80, 80, 80],
                'boundary_conditions': [['REFLECTION', 'ZERO_FLUX'],['REFLECTION', 'ZERO_FLUX'],['REFLECTION', 'ZERO_FLUX']],
...
params_model = {'layers':args.n_neuron,'activation':args.activation,'device':device,
                'fourier_mapping_size':None,'hard_BC':'TWIGL_3D'}

params_solver = {'scaling_loss':args.scaling_loss,
                'momentum':False,'beta1':0.0,'beta2':0.6,
                'num_inner_iters':2000, 'keff_ref':0.88598,'verbose':2, 'save_dir':save_dir,
...
pdeData.generate_phi_test(load_dir = data_dir+'TWIGL_3D/RefFlux50',multigroup=True)
pdeData.generate_p_test(load_dir = data_dir+'TWIGL_3D/RefCurrents50',multigroup=True)




geometry = CartesianGeometry(ndim=2,xmin=[0,0,0],xmax=[80, 80, 80],num_cells=[3,3,3],device=device)
geometry = geometry.from_edges(edges=[[0,24,56,80],[0,24,56,80],[0,24,56,80]],device=device)
# geometry.view_grid()
