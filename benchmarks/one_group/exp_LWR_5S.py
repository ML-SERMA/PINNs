...
params_pde = {'func_D':func_D,'func_Sigma_r':func_Sigma_r,'func_Sigma_s':func_Sigma_s,'func_Sf':func_Sf, 'num_groups':1}
...
visualization.viewCrossSection(pdeData.X_train,func_Sf(pdeData.X_train),pathFile=save_dir+'Source.png')
...
show_flux = True
if show_flux:
    phi_AE, phi_pred, phi_test= mypde.get_phi_test(pdeData.X_test,pdeData.phi_test,normalization=False)[2:5]
    for igroup in range(ngroup):
...
show_currents = True
if show_currents:
    p_AE, p_pred, p_test = mypde.get_currents_test(pdeData.Xc_test,pdeData.p_test,normalization=False)[2:5]
    for igroup in range(ngroup):
