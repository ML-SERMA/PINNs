...
parser.add_argument('-t','--test', type=str, help="name of test case",default='DDM_1x2_Mixed_FCN_Constant_D1_ns1_it20000_LR_1m3_theta_0_0')
...
parser.add_argument('-nt','--n_test',nargs='+', type=int, help="an integer number",default=[120,60])
...
nsubdo = 2
params_DDM = {'nsubdo':nsubdo, 'gridDDM':[1,2]}


params_domain = [{'xmin':[0, 0], 'xmax':[1, 0.5],'boundary_conditions': [['ZERO_FLUX', 'ZERO_FLUX'],['ZERO_FLUX', 'INTERFACE']]},
                {'xmin':[0, 0.5], 'xmax':[1, 1],'boundary_conditions': [['ZERO_FLUX', 'ZERO_FLUX'],['INTERFACE', 'ZERO_FLUX']]},
                ]
...
train_DDM(gridDDM=[1,2],subdo_pde=subdo_pde,subdo_data=subdo_data,params_train=params_train,n_step=20000)



for iDD in range(nsubdo):
    phi_pred_iDD = subdo_pde[iDD].phi_predict(subdo_data[iDD].X_test)
    visualization.viewSolution(params_domain[iDD],[120,60],phi_pred_iDD,save_dir+'phi_'+str(iDD)+'.png')
...
phi_pred = (pdeData.X_test[:,1:2]<=0.5)*subdo_pde[0].model.forward(pdeData.X_test)[:,0:1] + (pdeData.X_test[:,1:2]>0.5)*subdo_pde[1].model.forward(pdeData.X_test)[:,0:1] 
