...
nsubdo = 4
params_DDM = {'nsubdo':nsubdo, 'gridDDM':[2,2]}


params_domain = [{'xmin':[0, 0], 'xmax':[0.5, 0.5],'boundary_conditions': [['ZERO_FLUX', 'INTERFACE'],['ZERO_FLUX', 'INTERFACE']]},
                {'xmin':[0.5, 0], 'xmax':[1, 0.5],'boundary_conditions': [['INTERFACE', 'ZERO_FLUX'],['ZERO_FLUX', 'INTERFACE']]},
                {'xmin':[0, 0.5], 'xmax':[0.5, 1],'boundary_conditions': [['ZERO_FLUX', 'INTERFACE'],['INTERFACE', 'ZERO_FLUX']]},
                {'xmin':[0.5, 0.5], 'xmax':[1, 1],'boundary_conditions': [['INTERFACE', 'ZERO_FLUX'],['INTERFACE', 'ZERO_FLUX']]},
                ]
...
params_model = [{'layers':args.n_neuron,'activation':args.activation,'device':device,'fourier_mapping_size':None,'hard_BC': 'mixed_L1_LD'},
                ...
                {'layers':args.n_neuron,'activation':args.activation,'device':device,'fourier_mapping_size':None,'hard_BC': 'mixed_L1_LU'},
                {'layers':args.n_neuron,'activation':args.activation,'device':device,'fourier_mapping_size':None,'hard_BC': 'mixed_L1_RU'},
                ]
...
##=========================================================
params_main_domain = {'xmin':[0., 0.], 'xmax':[1, 1]}
...
subdo_pde[3].g_bc = [[None,None],[None,None]]
...
for iDD in range(nsubdo):
    ...
    visualization.viewSolution(params_domain[iDD],[60,60],phi_pred_iDD,save_dir+'phi_'+str(iDD)+'.png')
...
phi_pred = (pdeData.X_test[:,0:1]<=0.5)*(pdeData.X_test[:,1:2]<=0.5)*subdo_pde[0].model.forward(pdeData.X_test)[:,0:1] +\
    (pdeData.X_test[:,0:1]>0.5)*(pdeData.X_test[:,1:2]<0.5)*subdo_pde[1].model.forward(pdeData.X_test)[:,0:1] +\
    (pdeData.X_test[:,0:1]<0.5)*(pdeData.X_test[:,1:2]>0.5)*subdo_pde[2].model.forward(pdeData.X_test)[:,0:1]  +\
    (pdeData.X_test[:,0:1]>=0.5)*(pdeData.X_test[:,1:2]>=0.5)*subdo_pde[3].model.forward(pdeData.X_test)[:,0:1] 
