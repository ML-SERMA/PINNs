...
parser.add_argument('-t','--test', type=str, help="name of test case",default='DDM_3x1_Mixed_FCN_Constant_D1_ns10_it20000_LR_1m3_theta_0.5_0.5')
...
parser.add_argument('-nt','--n_test',nargs='+', type=int, help="an integer number",default=[40,120])
parser.add_argument('-ns','--n_step', type=int, help="an integer number",default=10)
...
nsubdo = 3
params_DDM = {'nsubdo':nsubdo, 'gridDDM':[3,1]}


params_domain = [{'xmin':[0, 0], 'xmax':[1.0/3, 1.0],'boundary_conditions': [['ZERO_FLUX', 'INTERFACE'],['ZERO_FLUX', 'ZERO_FLUX']]},
                {'xmin':[1.0/3, 0], 'xmax':[2.0/3, 1.0],'boundary_conditions': [['INTERFACE', 'INTERFACE'],['ZERO_FLUX', 'ZERO_FLUX']]},
                {'xmin':[2.0/3, 0], 'xmax':[1.0, 1.0],'boundary_conditions': [['INTERFACE','ZERO_FLUX'],['ZERO_FLUX', 'ZERO_FLUX']]},
                ]
...
# print(f"==>> subdo_data 0: {subdo_data[0].X_bc[0][1]}")
# print(f"==>> subdo_data 1-a: {subdo_data[1].X_bc[0][0]}")
# print(f"==>> subdo_data 1-b: {subdo_data[1].X_bc[0][1]}")
# print(f"==>> subdo_data 2: {subdo_data[2].X_bc[0][0]}")
# input('Enter')
...
subdo_pde[2].g_bc = [[None, None],[None,None]]





train_DDM(gridDDM=[3,1],subdo_pde=subdo_pde,subdo_data=subdo_data,params_train=params_train,n_step=20000)



for iDD in range(nsubdo):
    phi_pred_iDD = subdo_pde[iDD].phi_predict(subdo_data[iDD].X_test)
    visualization.viewSolution(params_domain[iDD],[40,120],phi_pred_iDD,save_dir+'phi_'+str(iDD)+'.png')
...
phi_pred = (pdeData.X_test[:,0:1]<=1.0/3)*subdo_pde[0].model.forward(pdeData.X_test)[:,0:1] +\
            (pdeData.X_test[:,0:1]>1.0/3)*(pdeData.X_test[:,0:1]<=2.0/3)*subdo_pde[1].model.forward(pdeData.X_test)[:,0:1] +\
            (pdeData.X_test[:,0:1]>2.0/3)*subdo_pde[2].model.forward(pdeData.X_test)[:,0:1] 
