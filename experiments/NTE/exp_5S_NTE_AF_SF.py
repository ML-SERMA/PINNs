...
parser.add_argument('-t','--test', type=str, help="name of test case",default='5S_NTE_AFSF_SLR1e-3_2000_095_SBC_uniform_angular_case3')
...
parser.add_argument('-nn','--n_neuron',nargs='+', type=int, help="list of integer number",default=[4]+5*[64]+[2])
...
pdeData.generate_angular_collocation_points(n_phi=10*1024,dim_angular=2,phi_rule='legendre')
pdeData.generate_angular_boundary_points(n_phi=512,dim_angular=2,phi_rule='legendre')
pdeData.combine_space_angular_points(mode='paired')
...
# Training Time
mypde = NeutronTransportAngularFluxScalarFlux(pdeDomain,model_fcn,params_pde,n_quad=n_quad,dim_angular=dim_angular,
...
# res_bc = mypde.residual_BC(pdeData.XS_bc)
# my_res_bc = mypde.my_residual_BC(pdeData.X_bc,pdeData.XS_bc)
...
scalar_flux, phi = mypde.compute_scalar_flux(pdeData.X_test)
...
for igroup in range(num_groups):
    ...
    visualization.viewSolution(params_domain,[96,86],phi[:,igroup],save_dir+'phi_group_'+str(igroup)+'.png')
