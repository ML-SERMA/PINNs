...
parser.add_argument('-t','--test', type=str, help="name of test case",default='Dauge_NTE_SLR1e-3_2000_095_SBC_new')
...
def Sigma_t_func(X):
    ...
    Dh = 10.0
    # R5 
    Sigma_t = (1.0/3*Dl)*torch.ones_like(x)
    Sigma_t[(x>=0.5)&(y<=0.5)] = 1.0/(3*Dh)
    Sigma_t[(x<=0.5)&(y>=0.5)] = 1.0/(3*Dh)
    return Sigma_t
...
def Sigma_s_func(X):
    ...
    # Dh = 10.0
    # # R5 
    # Sigma_s = (1-1.0/(3*Dl))*torch.ones_like(x)
    # Sigma_s[(x>=0.5)&(y<=0.5)] = 1-1.0/(3*Dh)
    # Sigma_s[(x<=0.5)&(y>=0.5)] = 1-1.0/(3*Dh)

    Sigma_s = torch.zeros_like(x)
