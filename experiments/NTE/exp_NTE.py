...
from pyPINNs.PDE_dev.neutron_transport_equation import NeutronTransportMultiOutput
...
parser.add_argument('-t','--test', type=str, help="name of test case",default='NTE_SLR1e-3_2000_095_HBC')
...
params_model = {'layers':args.n_neuron,'activation':args.activation,'device':device,
                'fourier_mapping_size':None,'hard_BC':'NTE_L1_G2'}


def Sigma_t_func(x):
    N = x.shape[0]
    return torch.tensor([[1.0, 0.5]], device=x.device).repeat(N, 1)  # shape [N, G]

def Sigma_s_func(x):
    N = x.shape[0]
    G = 2
    S = torch.zeros((N, G, G), device=x.device)
    S[:, 0, 0] = 0.8
    S[:, 0, 1] = 0.0
    S[:, 1, 1] = 0.3
    return S  # shape [N, G, G]

def q_func(x):
    N = x.shape[0]
    return torch.ones((N, 2), device=x.device) * 0.2  # External source [N, G]
...
phi_pred = [scalar_flux[:,0:1],scalar_flux[:,1:2]]
