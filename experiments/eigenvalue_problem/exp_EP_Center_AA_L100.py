...
from pyPINNs.PDE_dev.mixed_one_group_diffusion_eigenvalue import mixed_one_group_diffusion_eigenvalue
...
parser.add_argument('-t','--test', type=str, help="name of test case",default='EP_Mixed_FCN_Center_L100_D5_MSLR1e-3_NIT4000_HBC_AA_beta0.8_m10')
...
params_domain = {'xmin':[0, 0], 'xmax':[100, 100],'boundary_conditions': [['ZERO_FLUX', 'ZERO_FLUX'],['ZERO_FLUX', 'ZERO_FLUX']]} # zeroflux 
...
params_solver = {'momentum':False,'beta1':0.0,'beta2':0.1,
                'num_inner_iters':4000, 'keff_ref':0.99797,'verbose':2,
                'anderson':True,'beta':0.8,'m':10} #0.8
...
def func_D(X):
    ...
    D[(x>=100/3)&(x<=200/3)&(y>=100/3)&(y<=200/3)] = Dh
    return D
...
pdeData.generate_phi_test(load_dir = data_dir+'EP_Center_L100_D5/RefFlux120')
pdeData.generate_p_test(load_dir = data_dir+'EP_Center_L100_D5/RefCurrents120')
