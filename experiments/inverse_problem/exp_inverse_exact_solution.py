...
parser.add_argument('-t','--test', type=str, help="name of test case",default='FCN_NN_Inverse_Exact_D10_MSLR1e-3_HBC')
...
parser.add_argument('-nn','--n_neuron',nargs='+', type=int, help="an integer number",default=[2]+4*[64]+[1])
...
# class func_Source(torch.nn.Module):
#     def __init__(self,device):
#         super().__init__()
#         self.a1 = torch.nn.Parameter(torch.tensor(1.0).float().to(device))
#         self.a2 = torch.nn.Parameter(torch.tensor(1.0).float().to(device))
#     def forward(self, X):
#         D = 1
#         sigma_a = 1
#         pi = torch.acos(torch.zeros(1)).item() * 2
#         return ( D*(self.a1*pi)**2 + D*(self.a2*pi)**2 + sigma_a ) * torch.sin(self.a1*pi*X[:,0:1]) * torch.sin(self.a2*pi*X[:,1:2])
    
# model_S = func_Source(device=device).to(device)



model_S = FCN(layers=[2,32,32,1],activation='Tanh',device=device,hard_BC =False,transform=False).to(device)


params_pde = {'func_D':func_D,'func_Sigma_a':func_Sigma_a,'func_Source':model_S}

pdeDomain = SquareDomain(**params_domain)
pdeData = DataSet(domain=pdeDomain,**params_data)
pdeData.generate_u_test(f_exact=func_exact)
...
parameters = [*mypde.model.parameters(), *model_S.parameters()]
...
S = model_S(pdeData.X_test)
S = S.cpu().detach().numpy()
visualization.viewSolution(params_domain,120,S,save_dir)
print(f'a1 = {model_S.a1}, a2 = {model_S.a2}')
