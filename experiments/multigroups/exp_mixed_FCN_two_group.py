...
parser.add_argument('-t','--test', type=str, help="name of test case",default='NTK_Mixed_FCN_two_group_SLR1e-3_4000_095_HBC')
...
parser.add_argument('-nt','--n_test',nargs='+', type=int, help="list of integer number",default=[140,140])
...
params_domain = {'xmin':[0, 0], 'xmax':[7, 7]}
...
params_model = {'layers':args.n_neuron,'activation':args.activation,'device':device,
                'fourier_mapping_size':None,'hard_BC':'two_group_mixed_zero_L7'}
...
def func_D1(X):
    ...
    y=X[:,1:2]
    # R2 
    D = 1.31*torch.ones_like(x)
    # R1
    D[(x>=1)&(x<=6)&(y>=1)&(y<=6)] = 1.269
    return D
...
def func_D2(X):
    ...
    y=X[:,1:2]
    # R2 
    D = 0.8695*torch.ones_like(x)
    # R1
    D[(x>=1)&(x<=6)&(y>=1)&(y<=6)] = 0.9328
    return D

def func_Sigma_a1(X):
    x=X[:,0:1]
    y=X[:,1:2]
    Sigma_a = torch.zeros_like(x)
    Sigma_a[(x>=1)&(x<=6)&(y>=1)&(y<=6)] = 7.86e-4
    return Sigma_a

def func_Sigma_a2(X):
    x=X[:,0:1]
    y=X[:,1:2]
    Sigma_a = 2.117e-4*torch.ones_like(x)
    Sigma_a[(x>=1)&(x<=6)&(y>=1)&(y<=6)] = 4.1e-3
    return Sigma_a
...
def func_Sigma_s12(X):
    x=X[:,0:1]
    y=X[:,1:2]
    Sigma_s = 1.018e-2*torch.ones_like(x)
    Sigma_s[(x>=1)&(x<=6)&(y>=1)&(y<=6)] = 7.368e-3
    return Sigma_s
...
for igroup in range(ngroup):
    visualization.viewSolution(params_domain,[140,140],phi_pred[igroup],save_dir+'flux_group_'+str(igroup)+'.png')
...
for igroup in range(ngroup):
    visualization.viewErrorAndSolution(params_domain,[140,140],phi_AE[igroup],phi_pred[igroup],phi_test[igroup],
...
if show_currents:
    for igroup in range(ngroup):
        ...
        visualization.viewErrorAndCurrents(params_domain,[140,140],p_AE[igroup],p_pred[igroup],p_test[igroup],
