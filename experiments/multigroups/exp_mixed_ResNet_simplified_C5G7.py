...
from pyPINNs.PDE_dev.mixed_two_group_diffusion_source import mixed_two_group_diffusion_source
...
from pyPINNs.Model.neuron_network.DRR_NN import DeepRitzResNN
...
parser.add_argument('-t','--test', type=str, help="name of test case",default='Mixed_ResNet_2G_C5G7_case1_SLR1e-3_4000_095_HBC')
...
parser.add_argument('-i','--in_N', type=int, help="an integer number",default=2)
parser.add_argument('-w','--width', type=int, help="an integer number",default=64)
parser.add_argument('-d','--depth', type=int, help="an integer number",default=2)
parser.add_argument('-o','--out_N', type=int, help="an integer number",default=6)
...
name_folder = args.test + '_nc' + str(args.n_collocation) + '_nb' + str(args.n_boundary) \
                + '_nt' + str(args.n_test) + '_ns' + str(args.n_step)\
                + '_i'+str(args.in_N)+'_w'+str(args.width) + '_d'+str(args.depth)+ '_o'+str(args.out_N) + '_' +str(args.activation)
...
# params_model = {'layers':args.n_neuron,'activation':args.activation,'device':device,'fourier_mapping_size':None,'hard_BC':'2G_C5G7'}

params_model = {'in_N':args.in_N,'width':args.width,'depth':args.depth,'out_N':args.out_N ,'activation':args.activation,'hard_BC':'2G_C5G7'}
...
model_fcn = DeepRitzResNN(**params_model) .to(device)
