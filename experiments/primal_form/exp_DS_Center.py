...
from pyPINNs.PDE_dev.DS_primal_one_group_diffusion_source import DS_primal_one_group_diffusion_source
...
parser.add_argument('-t','--test', type=str, help="name of test case",default='DS_Primal_FCN_Center_D10_SLR1e-3_2000_095_HBC')
...
parser.add_argument('-ns','--n_step', type=int, help="an integer number",default=40000)
parser.add_argument('-log','--log_every', type=int, help="an integer number",default=100)
parser.add_argument('-nn','--n_neuron',nargs='+', type=int, help="an integer number",default=[2]+5*[64]+[2])
...
params_train = {'n_step':args.n_step,'verbose':args.verbose,'log_every':args.log_every,'LossFile':'Loss.csv','save_dir':save_dir,
                ...
                'weight_ridge':0,'weight_lasso':0,'weight_Sobolev':0, 'weight_interface':0,'resampling_every':0}

params_model = {'layers':args.n_neuron,'activation':args.activation,'device':device,
                'fourier_mapping_size':None,'hard_BC':'primal_L1_D'}
...
mypde = DS_primal_one_group_diffusion_source(pdeDomain,model_fcn,params_pde,device)
