...
from pyPINNs.PDE_dev.PC_primal_one_group_diffusion_source import PC_Dauge_primal_one_group_diffusion_source
...
parser.add_argument('-t','--test', type=str, help="name of test case",default='PC_Primal_FCN_Dauge_D2_SLR1e-3_2000_095_HBC')
...
parser.add_argument('-nn','--n_neuron',nargs='+', type=int, help="an integer number",default=[2]+7*[64]+[2])
...
params_train = {'n_step':args.n_step,'verbose':args.verbose,'log_every':args.log_every,'LossFile':'Loss.csv','save_dir':save_dir,
                ...
                'weight_ridge':0,'weight_lasso':0,'weight_Sobolev':0, 'weight_interface':1.0,'resampling_every':0}

params_model = {'layers':args.n_neuron,'activation':args.activation,'device':device,
                'fourier_mapping_size':None,'hard_BC':'primal_L1_PC'}

pdeDomain = SquareDomain(**params_domain)
pdeData = DataSet(domain=pdeDomain,**params_data)
pdeData.generate_phi_test(load_dir = data_dir+'Dauge_D2/RefFlux120')
# pdeData.generate_p_test(load_dir = None)
# pdeData.prepare_DataLoader(batch_size_collocation=None,batch_size_boundary=None)

pdeData.generate_interface_points(n_face=512,test='Dauge')
visualization.viewCrossSection(pdeData.X_interface[0],S=None,pathFile=save_dir+'interface_0.png')
visualization.viewCrossSection(pdeData.X_interface[1],S=None,pathFile=save_dir+'interface_1.png')
...
mypde = PC_Dauge_primal_one_group_diffusion_source(pdeDomain,model_fcn,params_pde,device)
