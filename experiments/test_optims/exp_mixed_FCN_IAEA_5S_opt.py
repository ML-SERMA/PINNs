...
from pyPINNs.Model.neuron_network.FCN import FCN, FCN_FF, FCN_FA
...
parser.add_argument('-t','--test', type=str, help="name of test case",default='Mixed_FCN_FA_IAEA_5S_MSLR1e-3_LBFGS_New')
...
params_model = {'layers':args.n_neuron,'activation':args.activation,'device':device,
                'fourier_mapping_size':None,'hard_BC':False,'transform':False}
...
pdeData.generate_u_test(load_dir = data_dir+'IAEA-5S/RefFlux_96_86')
...
# params_train['n_step']= 150000
# optimizer = torch.optim.Adam(mypde.model.parameters(), lr=1.e-3,weight_decay=0.0)
# scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[20000,40000,60000,80000,100000,150000], gamma=0.2)
# mypde.compile(optimizer=optimizer,scheduler=scheduler)
# train(mypde,pdeData,**params_train)

# continue to train with LBFGS
params_train['n_step']= 50000
params_train.update({'mode':'a'})

optimizer_LBFGS = torch.optim.LBFGS(mypde.model.parameters(),lr=0.6,line_search_fn="strong_wolfe")
...
# Visualization
visualization.viewErrorAndSolution(params_domain,[96,86],error_absolute,u_pred,u_test,save_dir)
