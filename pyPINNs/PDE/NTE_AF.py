...
class NeutronTransportAngularFlux(pdeBase):
    ...
    def residual_PDE(self, x):
        ...
        psi_flat = self.model(x) # [N_in, G]
        x_in = x[:,0:self.dim_angular] # [N_in, d]
        s_in = x[:,self.dim_angular:] # [N_in, d]

        # Compute gradients
        grads = [torch.autograd.grad(psi_flat[:, g].sum(), x, create_graph=True, retain_graph=True,allow_unused=True)[0] for g in range(G)] # list of [N_in, d]
        # Replace None (disconnected grads) with zeros
    ...
    def residual_BC(self, x_b_list):
        ...
        for dim in range(len(x_b_list)):
            for side in range(2):  # 0: min side, 1: max side
                xs_bc = x_b_list[dim][side]  # [N_in, d+d]
                x_bc = xs_bc[:,0:self.dim_angular] # [N_in, d]
                s_bc = xs_bc[:,self.dim_angular:] # [N_in, d]
                psi_pred = self.model(xs_bc)                # [N_in, G]

                # Compute normal vector
                n_hat = torch.zeros_like(x_bc, device=self.device) # [N_in, d]
                n_hat[:, dim] = 1.0 if side == 1 else -1.0

                # dot_prod = torch.einsum('nd,nd->n', s_bc, n_hat)  # [N_in]
                dot_prod = torch.sum(s_bc * n_hat, dim=1)             # [N_in]
                inflow_mask = (dot_prod < 0).unsqueeze(-1)  # [N_in, 1]
                psi_inflow = psi_pred * inflow_mask         # [N_in, G]
                residuals.append(psi_inflow)

        return torch.cat(residuals, dim=0)  # [total_N,  G]
